...
function injectEventMeta(event: any) {
  ...
  const description = event.description
    ...
    : `${event.type} · ${event.date}${event.time ? ' at ' + event.time : ''} · ${
        event.location
      }. Curated by Spa-Pregio®.`;
    ...
    },
    {
      ...
      icon: (
        <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
          <path
            d="M2 2h4l2.5 3.5L11 2h3l-4.5 6L14 14h-4L7.5 10 5 14H2l4.5-6L2 2z"
            fill="white"
          />
        </svg>
      ),
...
export default function EventDetail() {
  ...
  const loadEvent = async () => {
    ...
    const attendeeOnly = (rsvps || []).filter((r: any) => !r.is_vendor);
    const vendorsOnly = (rsvps || []).filter((r: any) => r.is_vendor);
