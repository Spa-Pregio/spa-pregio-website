...
function injectEventMeta(event: any) {
  ...
  const description = event.description
    ...
    : `${event.type} · ${event.date}${event.time ? ' at ' + event.time : ''} · ${event.location}. Curated by Spa-Pregio®.`;
  document.title = `${event.title} — Spa-Pregio®`;
  ...
  setMeta('og:site_name', 'Spa-Pregio®');
...
function ShareRow({ event }: { event: any }) {
  ...
  const eventUrl = `https://spa-pregio.com/events/${event.id}`;
  const shareText = encodeURIComponent(`Join me at "${event.title}" — ${event.date} · ${event.location} · Curated by Spa-Pregio®`);
  const encodedUrl = encodeURIComponent(eventUrl);
  const encodedImage = encodeURIComponent(event.image || 'https://spa-pregio.com/images/gathering_large.jpg');

  const platforms = [
    { name: 'Facebook', href: `https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`, bg: '#1877F2', icon: <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M9.5 3h1.5V1h-1.5C8.1 1 7 2.1 7 3.5V5H5.5v2H7v6h2V7h1.5l.5-2H9V3.5c0-.3.2-.5.5-.5z" fill="white"/></svg> },
    { name: 'Pinterest', href: `https://pinterest.com/pin/create/button/?url=${encodedUrl}&description=${shareText}&media=${encodedImage}`, bg: '#E60023', icon: <svg width="15" height="15" viewBox="0 0 16 16" fill="none"><path d="M8 1C4.1 1 1 4.1 1 8c0 2.9 1.7 5.4 4.1 6.6-.1-.8-.1-2 .2-2.9l1.2-5s-.3-.6-.3-1.5c0-1.4.8-2.4 1.8-2.4.9 0 1.3.7 1.3 1.5 0 .9-.6 2.3-.9 3.5-.2 1 .5 1.8 1.5 1.8 1.8 0 2.9-2.3 2.9-5.1 0-2.1-1.4-3.7-3.9-3.7-2.8 0-4.5 2.1-4.5 4.4 0 .8.2 1.4.6 1.8.1.1.1.2.1.3-.1.3-.2.9-.2.9 0 .1-.1.2-.2.1C3.8 9.7 3 8.3 3 6.7 3 3.9 5.4 1 8.3 1c2.9 0 4.7 2.1 4.7 4.4 0 3.4-1.9 5.9-4.6 5.9-1 0-1.9-.5-2.2-1.1l-.6 2.2c-.2.8-.7 1.7-1.1 2.3.8.3 1.7.4 2.5.4 3.9 0 7-3.1 7-7S11.9 1 8 1z" fill="white"/></svg> },
    { name: 'X', href: `https://twitter.com/intent/tweet?text=${shareText}&url=${encodedUrl}`, bg: '#000000', icon: <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M2 2h4l2.5 3.5L11 2h3l-4.5 6L14 14h-4L7.5 10 5 14H2l4.5-6L2 2z" fill="white"/></svg> },
    { name: 'TikTok', href: `https://www.tiktok.com/share?url=${encodedUrl}`, bg: '#010101', icon: <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M13 1h-2.5v9.5a2 2 0 1 1-2-2 2 2 0 0 1 .5.1V6a4.5 4.5 0 1 0 4 4.5V5.5A6 6 0 0 0 13 6V1z" fill="white"/></svg> },
  ];

  const handleCopy = async () => {
    try { await navigator.clipboard.writeText(eventUrl); } catch {
      const el = document.createElement('textarea'); el.value = eventUrl;
  ...
  return (
    <div className="mt-6 pt-6 border-t border-spa-charcoal/5">
      <p className="text-xs uppercase tracking-[0.12em] text-spa-gray mb-3 font-medium">Share this event</p>
      <div className="flex items-center gap-2 mb-3">
        {platforms.map((p) => (
          <a key={p.name} href={p.href} target="_blank" rel="noopener noreferrer" title={`Share on ${p.name}`}
            ...
            style={{ background: p.bg }}>{p.icon}</a>
        ))}
      </div>
...
export default function EventDetail() {
  ...
  useEffect(() => { window.scrollTo(0, 0); loadEvent(); checkUser(); }, [id]);
  ...
  const loadEvent = async () => {
    ...
    setEvent(data); injectEventMeta(data);
    const { data: rsvps } = await supabase.from('event_rsvps').select('id, is_vendor').eq('event_id', String(id));
  ...
  const getTotalPrice = () => {
    ...
    return tickets.reduce((total: number, ticket: any, index: number) =>
      total + Number(ticket?.price || 0) * (selectedTickets[index] || 0), 0);
  };
  ...
  const handleFreeRsvp = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('event_rsvps').insert([{
      event_id: String(event.id), user_email: rsvpEmail, user_name: rsvpName, is_vendor: false,
    }]);
    if (error) { setRsvpStatus('error'); } else { setRsvpStatus('success'); setRsvpCount((c) => c + 1); }
  };
  ...
  const handlePaidCheckout = async () => {
    ...
    try {
      const ticketsPayload = (Array.isArray(event.tickets) ? event.tickets : [])
        .map((t: any, i: number) => ({ type: t.type, price: t.price, description: t.description, quantity: selectedTickets[i] || 0 }))
      ...
      else { console.error('Checkout error:', data.error); setPaymentStatus('error'); }
    } catch (err) { console.error('Checkout exception:', err); setPaymentStatus('error'); }
  };
    ...
    try {
      const response = await fetch(`${SUPABASE_FUNCTIONS_URL}/create-checkout-session`, {
        ...
        body: JSON.stringify({
          ...
          tickets: [{ type: 'Vendor Table', price: Number(event.vendor_table_price || 0), description: `Vendor table at ${event.title}`, quantity: 1 }],
          connected_account_id: event.connected_account_id || '',
          customer_email: rsvpEmail,
          customer_name: rsvpName,
        }),
      ...
      const data = await response.json();
      if (data.url) { window.location.href = data.url; }
      else { console.error('Vendor checkout error:', data.error); setVendorPaymentStatus('error'); }
    } catch (err) { console.error('Vendor checkout exception:', err); setVendorPaymentStatus('error'); }
  };
  ...
  if (notFound || !event) return (
    <div className="w-full pt-20 min-h-screen bg-spa-cream flex items-center justify-center">
      <div className="text-center max-w-md px-6">
        ...
        <p className="text-spa-gray mb-6">This event may have ended or the link may be incorrect.</p>
        <Link to="/events" className="btn-primary justify-center"><ArrowLeft size={18} /> Browse all events</Link>
      </div>
    </div>
  );

  const tickets = Array.isArray(event?.tickets) ? event.tickets : [];
  const minPrice = tickets.length > 0 ? Math.min(...tickets.map((t: any) => Number(t?.price || 0))) : 0;
  ...
  return (
    <div className="w-full pt-20 min-h-screen bg-spa-cream">
      <div className="max-w-3xl mx-auto px-6 lg:px-12 pt-8">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-sm text-spa-gray hover:text-spa-purple transition-colors mb-8">
      ...
      <div className="max-w-3xl mx-auto px-6 lg:px-12 pb-24">
        <div className="bg-white rounded-2xl overflow-hidden shadow-elegant">
          <div className="relative aspect-[16/7] overflow-hidden bg-spa-lavender">
            <img src={event.image || '/images/gathering_large.jpg'} alt={event.title} className="w-full h-full object-cover" />
          ...
          <div className="p-6 lg:p-10">
            ...
            <h1 className="font-serif text-3xl lg:text-4xl text-spa-charcoal mt-2 mb-5 leading-tight">{event.title}</h1>
            <div className="space-y-2 mb-6">
              <div className="flex items-center gap-2 text-sm text-spa-gray"><Calendar size={16} className="text-spa-purple flex-shrink-0" />{event.date}{event.time ? ` · ${event.time}` : ''}</div>
              <div className="flex items-center gap-2 text-sm text-spa-gray"><MapPin size={16} className="text-spa-purple flex-shrink-0" />{event.location}</div>
              <div className="flex items-center gap-2 text-sm text-spa-gray"><Users size={16} className="text-spa-purple flex-shrink-0" />{rsvpCount} / {event.max_attendees || 50} attending</div>
            </div>
            {event.description && <p className="text-spa-gray leading-relaxed mb-2">{event.description}</p>}
              ...
              <div className="mt-6 pt-6 border-t border-spa-charcoal/5">
                <div className="bg-spa-lavender rounded-2xl p-6">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div>
                      <h4 className="font-serif text-xl text-spa-charcoal flex items-center gap-2"><Store size={18} className="text-spa-purple" />Vendor Tables</h4>
                      <p className="text-sm text-spa-gray mt-1">Showcase your business at this event.</p>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-semibold text-spa-purple">${Number(event.vendor_table_price).toFixed(2)}</p>
                      <p className="text-xs text-spa-gray">{vendorTablesRemaining} of {vendorTablesAvailable} left</p>
                    </div>
                  </div>
                  {!currentUser ? (
                    <div className="mt-4">
                      <p className="text-sm text-spa-gray mb-3">Create a free account to reserve a vendor table.</p>
                      <Link to="/join" className="btn-primary justify-center">Create Free Account</Link>
                    </div>
                  ) : vendorPaymentStatus === 'success' ? (
                    <div className="mt-4 bg-white rounded-xl p-4">
                      <div className="flex items-center gap-2 text-spa-purple font-medium"><Check size={18} /> Vendor table reserved</div>
                      <p className="text-sm text-spa-gray mt-1">Your vendor reservation has been recorded for {event.title}.</p>
                    </div>
                  ) : (
                    <div className="mt-4 space-y-4">
                      {vendorPaymentStatus === 'error' && (
                        <div className="flex items-center gap-2 text-red-500 text-sm bg-red-50 px-4 py-3 rounded-xl"><AlertCircle size={16} />Payment failed. Please try again.</div>
                      )}
                      <button onClick={handleVendorTableCheckout} disabled={vendorTablesRemaining <= 0 || vendorPaymentStatus === 'loading'} className="btn-primary w-full justify-center disabled:opacity-50">
                        {vendorTablesRemaining <= 0 ? 'Vendor Tables Sold Out' : vendorPaymentStatus === 'loading' ? 'Redirecting to checkout...' : `Reserve Vendor Table — $${Number(event.vendor_table_price).toFixed(2)}`}
                      </button>
                    </div>
                  )}
                </div>
            ...
            <div className="mt-6 pt-6 border-t border-spa-charcoal/5">
              {!currentUser ? (
                ...
                <div className="text-center py-8 bg-spa-lavender rounded-2xl">
                  <div className="w-14 h-14 rounded-full bg-spa-purple/10 flex items-center justify-center mx-auto mb-4"><Check size={26} className="text-spa-purple" /></div>
                ...
                <form onSubmit={handleFreeRsvp} className="space-y-4">
                  <h4 className="font-serif text-xl text-spa-charcoal flex items-center gap-2"><Ticket size={18} className="text-spa-purple" /> RSVP — Free Event</h4>
                  <div>
                ...
                <div className="space-y-4">
                  <h4 className="font-serif text-xl text-spa-charcoal flex items-center gap-2"><Ticket size={18} className="text-spa-purple" /> Select Tickets</h4>
                  ...
                  <button onClick={handlePaidCheckout} disabled={getTotalPrice() === 0 || paymentStatus === 'loading'} className="btn-primary w-full justify-center disabled:opacity-50">
                    {paymentStatus === 'loading' ? 'Redirecting to checkout...' : getTotalPrice() === 0 ? 'Select tickets above' : `Pay $${getTotalPrice()}`}
                    {paymentStatus !== 'loading' && getTotalPrice() > 0 && <DollarSign size={18} />}
                  </button>
                  <p className="text-xs text-spa-gray text-center">You'll be redirected to Stripe's secure checkout page.</p>
                </div>
              )}
            </div>
          </div>
