...
function ShareRow({ event }: { event: any }) {
  ...
  const shareText = encodeURIComponent(
    `Join me at "${event.title}" — ${event.date} · ${event.location} · Curated by Spa-Pregio®`
  );
  ...
  const platforms = [
    {
      ...
      icon: (
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <path d="M9.5 3h1.5V1h-1.5C8.1 1 7 2.1 7 3.5V5H5.5v2H7v6h2V7h1.5l.5-2H9V3.5c0-.3.2-.5.5-.5z" fill="white"/>
        </svg>
      ),
    },
    {
    ...
    },
    {
      ...
      icon: (
        <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
          <path d="M13 1h-2.5v9.5a2 2 0 1 1-2-2 2 2 0 0 1 .5.1V6a4.5 4.5 0 1 0 4 4.5V5.5A6 6 0 0 0 13 6V1z" fill="white"/>
        </svg>
      ),
    },
  ];
...
export default function EventDetail() {
  ...
  const getTotalPrice = () => {
    ...
    return event.tickets.reduce((total: number, ticket: any, index: number) => {
  ...
  const handleFreeRsvp = async (e: React.FormEvent) => {
    ...
    else { setRsvpStatus('success'); setRsvpCount(c => c + 1); }
  };
      ...
      <div className="max-w-3xl mx-auto px-6 lg:px-12 pb-24">
        <div className="bg-white rounded-2xl overflow-hidden shadow-elegant">
          ...
          <div className="p-6 lg:p-10">
            ...
            {/* Share row — fully functional with real URL */}
              ...
              {!currentUser ? (
                ...
                <form onSubmit={handleFreeRsvp} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text" required value={rsvpName}
                  ...
                  <div>
                    ...
                    <input
                      type="email" required value={rsvpEmail}
                ...
                <div className="space-y-4">
                    ...
                    <div key={index} className="flex items-center justify-between p-4 bg-spa-lavender rounded-xl">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() => setSelectedTickets({ ...selectedTickets, [index]: Math.max(0, (selectedTickets[index] || 0) - 1) })}
                          ...
                          <button
                            type="button"
                            onClick={() => setSelectedTickets({ ...selectedTickets, [index]: (selectedTickets[index] || 0) + 1 })}
                            className="w-7 h-7 rounded-full bg-white flex items-center justify-center text-spa-charcoal hover:bg-spa-purple hover:text-white transition-colors font-bold"
                          >+</button>
                        </div>
                      </div>
                  ...
                  >
                    {paymentStatus === 'loading' ? 'Processing...'
                      : getTotalPrice() === 0 ? 'Select tickets above'
                      : `Pay $${getTotalPrice()}`}
